...
public class Controller2D : RaycastController {
    ...
    private bool notOverLedge;
    private bool isTouchingWall;
    private bool ledgeDetected = false;
    private Vector2 currentBottomOrigin;
	...
	void HorizontalCollisions(ref Vector2 moveAmount) {
		...
		for (int i = 0; i < horizontalRayCount; i ++) {
            ...
            isTouchingWall = Physics2D.Raycast(rayOrigin, Vector2.right * directionX, rayLength, collisionMask);
            notOverLedge = Physics2D.Raycast(rayOrigin + Vector2.up * (horizontalRaySpacing * (horizontalRayCount - 1)), Vector2.right * directionX, rayLength, collisionMask);

            if(isTouchingWall && !notOverLedge && !ledgeDetected)
            {
    ...
    void ClimbLedge()
    {
        Vector2 rayOrigin = (collisions.faceDir == -1) ? raycastOrigins.bottomLeft : raycastOrigins.bottomRight;
        float yOffset = (rayOrigin + Vector2.up * (horizontalRaySpacing * (horizontalRayCount - 1))).y - rayOrigin.y;
        yOffset *= 1.5f;
        ...
        if(ledgeDetected && !canClimbLedge)
        {
            ...
            {
                ledgePos1 = new Vector2(Mathf.Ceil(currentBottomOrigin.x + skinWidth) + transform.localScale.x * 0.85f, Mathf.Floor(currentBottomOrigin.y) + yOffset);
            }
            else
            {
                ledgePos1 = new Vector2(Mathf.Ceil(currentBottomOrigin.x + skinWidth) - transform.localScale.x * 0.85f, Mathf.Floor(currentBottomOrigin.y) + yOffset);
